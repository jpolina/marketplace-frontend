...
function AdForm() {
    ...
    const onChange = (e) => {
        ...
        if (e.target.name=='price') setFormData({...formData, [e.target.name]:parseInt(e.target.value)})
    }

    const onSubmit = async (e) => {
        e.preventDefault();
        const token = await JSON.parse(localStorage.getItem('seller')).token
        const finalData = await {...formData, ['price']:formData['price']*100}
        const config = {
            headers: {
        ...
        try {
            const response = await axios.post(API_URL+'ad', finalData, config)
